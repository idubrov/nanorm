...
public class InternalConfiguration {
    ...
    private void processMethod(Class<?> mapper, Method method) throws ConfigurationException {
        ...
        if (select != null) {
            ...
            // Skip method
            LOGGER.info("Skipping method '{}' in mapper '{}' (no configuration).", method
                    .getName(), mapper.getName());
            return;
        }
        ...
        if (selectKey != null) {
            ...
            StatementKey selectKeyKey = new StatementKey(mapper, method.getName() + ":key",
            ...
            if (keyType == void.class) {
                ...
                keyType = introspectionFactory.getParameterType(
                        method.getGenericParameterTypes(), prop);
            }
    ...
    private RowMapper createRowMapper(Type type, ResultMapConfig config) {
        ...
        if (config.isScalar()) {
            if (type instanceof Class<?> && ((Class<?>) type).isArray()) {
                return new ScalarRowMapper(((Class<?>) type).getComponentType(),
                        typeHandlerFactory);
            }
    ...
    private PropertyMappingConfig createPropertyMappingConfig(Class<?> mapper, Method method,
        ...
        if (!"".equals(mapping.subselect())) {
            ...
            // class for this...
            StatementKey subselectKey = new StatementKey(subselectMapper, mapping.subselect(),
                    null);
