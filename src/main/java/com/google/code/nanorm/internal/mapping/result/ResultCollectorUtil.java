 ...
 * creating {@link DataSinkSource} to be used fo pushing the results into the
 * property.
 * 
...
public class ResultCollectorUtil {
     ...
     */
    public static DataSinkSource createDataSinkSource(Getter getter, Setter setter,
     ...
     * 
     * For example, result bean type for <code>java.util.List&lt;Bean&gt</code>
     * will be <code>Bean</code>.
     * 
    ...
    public static Class<?> resultClass(Type resultType) {
        ...
        if (resultClass == null) {
            throw new IntrospectionException("Result type not supported! Got type " + resultType);
        }
        return resultClass;
    }
